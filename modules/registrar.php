<?php
// check wordpress network url, if not registrar.utm.my, return
if (strpos($_SERVER['HTTP_HOST'], 'registrar.utm.my') === false) {
	return;
}
...
{
	global $post;
	if (!isset($post) || !is_a($post, 'WP_Post') || strpos($post->post_name, 'career') === false) {
		return;
	}
...
// This function will check if the URL contains 'career' and add check with JSON file.
// If user email does not exist in the JSON file, display notice to ask user to login view www.utm.my/login. 
// Once login, user can refresh the page
function registrar_check_user_login($content)
{
	$url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";	
	if (strpos($url, 'borang-permohonan-tncaa') !== false) {
