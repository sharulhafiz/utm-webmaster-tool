...
/*==================================================================================
  wp_mail Anti-spam Filter — Blocks spammy emails from contact forms
==================================================================================*/

/**
 * Hook into wp_mail to check outgoing emails for spam content.
 * Catches contact form submissions that bypass the comment pipeline
 * (Divi Contact Form, other custom forms, etc.)
 */
add_filter('wp_mail', 'utm_antispam_check_mail', 10, 1);

function utm_antispam_check_mail($args) {
    // Don't block notification/alert emails from our own system
    if (isset($args['subject']) && (
        strpos($args['subject'], 'Spam') !== false ||
        strpos($args['subject'], 'UTM Anti-Spam') !== false ||
        strpos($args['subject'], 'Database Alert') !== false
    )) {
        return $args;
    }
    ...
    if (!empty($spam_words)) {
        foreach ($spam_words as $word) {
            if (strpos($content_lower, strtolower($word)) !== false) {
                error_log('[UTM AntiSpam] Blocked wp_mail — word match: "' . $word . '" | Subject: ' . ($args['subject'] ?? ''));
                return false;
            }
        }
    }

    // 2. Check for Cyrillic/non-Latin content in emails from contact forms
    // Contact form emails on UTM sites should be in Malay/English
    $from_check = '';
    if (isset($args['headers'])) {
        $headers = is_array($args['headers']) ? implode("\n", $args['headers']) : $args['headers'];
        $from_check = $headers;
    }
    // Only check for non-Latin if it's a contact form (From: contains mail@)
    if (preg_match('/From:.*?mail@/i', $from_check)) {
        // Check if message has significant non-Latin characters (Cyrillic, Arabic, etc.)
        $non_latin = preg_replace('/[a-zA-Z0-9\s.,!?\'"@()\-_\/:;+=#\[\]{}<>]/', '', $content);
        $non_latin = trim(preg_replace('/[«»\—\–\""\'\']/', '', $non_latin));
        if (strlen($non_latin) > 20) {
            error_log('[UTM AntiSpam] Blocked wp_mail — non-Latin content detected | Subject: ' . ($args['subject'] ?? ''));
            return false;
        }
    }

    // 3. Check for suspicious URLs with spammy TLDs or patterns
    if (preg_match_all('/https?:\/\/([^\s<>"\']+)/i', $content, $urls)) {
        $spammy_tlds = array('.market', '.click', '.loan', '.work', '.date', '.men', '.win', '.bid');
        $spammy_keywords = array('casino', 'bet', 'poker', 'slot', 'gambling', 'crypt-o', 'binary');

        foreach ($urls[0] as $url) {
            $url_lower = strtolower($url);
            foreach ($spammy_tlds as $tld) {
                if (strpos($url_lower, $tld) !== false) {
                    error_log('[UTM AntiSpam] Blocked wp_mail — suspicious TLD (' . $tld . ') in URL | Subject: ' . ($args['subject'] ?? ''));
                    return false;
                }
            }
            foreach ($spammy_keywords as $kw) {
                if (strpos($url_lower, $kw) !== false) {
                    error_log('[UTM AntiSpam] Blocked wp_mail — suspicious keyword (' . $kw . ') in URL | Subject: ' . ($args['subject'] ?? ''));
                    return false;
                }
            }
        }
    ...
    // 4. Check for excessive URL count (spam indicator)
    if (preg_match_all('/https?:\/\//i', $content, $url_count) > 3) {
        error_log('[UTM AntiSpam] Blocked wp_mail — excessive URLs (' . count($url_count[0]) . ') | Subject: ' . ($args['subject'] ?? ''));
        return false;
    }
