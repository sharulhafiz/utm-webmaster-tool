...
// NOTE: sso_login_support_user() removed in SSO Improvement Plan (Phase 1.2)
...
function sso_send_pin() {
    ...
    // Rate limit: 60s cooldown per email address (Phase 2.1)
    $throttle_key = 'sso_pin_throttle_' . md5($email);
    $last_sent = get_transient($throttle_key);
    if ($last_sent) {
        $wait = 60 - (time() - $last_sent);
        wp_send_json_error( sprintf( 'Please wait %d seconds before requesting another PIN.', $wait ) );
    }
    ...
    // Log PIN request (Phase 4.2)
    error_log(sprintf(
        '[SSO] PIN requested | email=%s | ip=%s | user_id=%d',
        $email, $user_ip, $user ? $user->ID : 0
    ));
    ...
    if ( ! $user ) {
        ...
        if ( $auto_create && in_array($email_domain, $allowed_domains) ) {
            ...
            $random_pass = wp_generate_password(24, false);
    ...
    // Store PIN in transient, not as the user's WP password (Phase 3.1)
    $fallback_password = wp_generate_password(24, false);
    wp_set_password($fallback_password, $user->ID);
    set_transient('sso_pin_' . $user->ID, wp_hash_password($pin), 300); // 5 min expiry

    $siteURL = rtrim(str_replace('https://', '', get_site_url()), '/');

    // if email is webmaster@utm.my, send code using telegram
    if (false) { // Disabled Telegram PIN - using email instead (May 08 2026)
        // Default token in code for deployment convenience (Phase 4.1)
        $telegram_token = '7728747017:***';
        // Allow override via wp option (for rotation without plugin update)
        $saved_token = get_option('sso_telegram_token');
        ...
        if (!empty($telegram['token']) && !empty($telegram['chat_id'])) {
            ...
            $response = wp_remote_get($url, array('timeout' => 5, 'blocking' => false));
            set_transient($throttle_key, time(), 60);
            wp_send_json_success( 'PIN sent successfully to ' . $user->user_email . ' - ' . $pin );
        }
    } else {
        $subject = $siteURL . ' - Your UTM Login PIN';
        $message = "Your one-time PIN code is: {$pin}\n\n" .
            "Use this PIN to log in to {$siteURL}.\n" .
            "Request IP: {$user_ip}\n\n" .
            "If you did not request this PIN, you can ignore this email.";
        $headers = array(
            'From: UTM SSO <webmaster@utm.my>',
            'Reply-To: webmaster@utm.my',
        );
        $mailstatus = wp_mail( $user->user_email, $subject, $message, $headers );
        if($mailstatus){
            set_transient($throttle_key, time(), 60);
            wp_send_json_success( 'PIN sent successfully to ' . $user->user_email );
        } else {
            // Log + Telegram alert on email failure (Phase 4.4)
            error_log(sprintf(
                '[SSO] EMAIL FAILURE | to=%s | ip=%s | user_id=%d',
                $email, $user_ip, $user->ID
            ));

            // Deduplicate: max 1 alert per 5 min
            $alert_key = 'sso_email_alert_last_sent';
            $last_alert = get_transient($alert_key);
            if (!$last_alert || $last_alert < time() - 300) {
                set_transient($alert_key, time(), 300);

                $telegram_token_alert = '7728747017:***';
                $saved_token_alert = get_option('sso_telegram_token');
                if (!empty($saved_token_alert)) {
                    $telegram_token_alert = $saved_token_alert;
                }

                $alert_message = urlencode(sprintf(
                    "🚨 SSO Email Failure — %s\n\n" .
                    "To: %s\nUser ID: %d\nIP: %s\nTime: %s\n\n" .
                    "PIN email delivery failed. Check msmtp/Gmail relay.",
                    get_site_url(), $email, $user->ID, $user_ip, current_time('mysql')
                ));
                $alert_url = "https://api.telegram.org/bot{$telegram_token_alert}/sendMessage?chat_id=-1002370420190&text={$alert_message}";
                wp_remote_get($alert_url, array('timeout' => 5, 'blocking' => false));
            }

            wp_send_json_error( sprintf( 'Could not deliver email to %s. Please check your address or contact support.', $user->user_email ) );
        }
    }
}
...
function sso_validate_pin() {
    ...
    // Rate limit: max 5 failed attempts per 15 minutes per email (Phase 2.2)
    $attempt_key = 'sso_pin_attempts_' . md5($email);
    $attempts = (int) get_transient($attempt_key);

    if ($attempts >= 5) {
        wp_send_json_error( 'Too many failed attempts. Please try again in 15 minutes.' );
    }
    ...
    // Check PIN against transient hash (Phase 3.1)
    $stored_hash = get_transient('sso_pin_' . $user->ID);
    $pin_valid = $stored_hash && wp_check_password( $pin, $stored_hash );
    ...
    if ( $pin_valid ) {
        ...
        // Clean up the PIN transient (single-use)
        delete_transient('sso_pin_' . $user->ID);
        ...
        // Log successful login (Phase 4.2)
        error_log(sprintf(
            '[SSO] Login success | email=%s | user_id=%d | ip=%s',
            $email, $user->ID, isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 'unknown'
        ));
        ...
        set_transient($attempt_key, $attempts + 1, 900); // 15 min window
        $remaining = 5 - ($attempts + 1);

        // Log failed attempt (Phase 4.2)
        error_log(sprintf(
            '[SSO] PIN failed | email=%s | ip=%s | attempt=%d/5',
            $email, isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 'unknown', $attempts + 1
        ));

        wp_send_json_error( sprintf( 'Invalid PIN. %d attempt(s) remaining.', $remaining ) );
    }
}
...
function utm_sso(){
    // Skip auto-login for actions that must be handled by wp-login.php directly.
