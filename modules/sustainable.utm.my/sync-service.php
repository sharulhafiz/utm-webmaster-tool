...
function utm_sustainable_upsert_page_from_google_doc( $payload ) {
    ...
    $force_menu_sync  = ! empty( $payload['force_menu_sync'] );
    $is_new_page      = false;
    $folder_path_json = wp_json_encode( $folder_path );

    utm_sustainable_set_last_upsert_result_meta(
        array(
            'action' => 'updated',
            'reason' => '',
        )
    ...
    if ( $post_id > 0 && '' !== $google_modified ) {
        $stored_modified = sanitize_text_field( (string) get_post_meta( $post_id, '_utm_google_doc_modified_at', true ) );

        if ( '' !== $stored_modified && hash_equals( $stored_modified, $google_modified ) ) {
            utm_sustainable_set_last_upsert_result_meta(
                array(
                    'action' => 'skipped',
                    'reason' => 'unchanged',
    ...
    $stored_folder_path_json = '';

    if ( $post_id > 0 ) {
        $stored_folder_path_json = (string) get_post_meta( $post_id, '_utm_sustainable_folder_path', true );
    }
    ...
    if ( $post_id > 0 ) {
        ...
        $is_new_page = ! is_wp_error( $result ) && $post_id > 0;
    }
    ...
    if ( '' !== $google_modified ) {
        update_post_meta( $post_id, '_utm_google_doc_modified_at', $google_modified );
    }

    update_post_meta( $post_id, '_utm_sustainable_folder_path', $folder_path_json );

    $folder_path_changed = ( (string) $stored_folder_path_json !== (string) $folder_path_json );
    $should_sync_menu    = $is_new_page || $folder_path_changed || $force_menu_sync;

    if ( $should_sync_menu && is_array( $folder_path ) && ! empty( $folder_path ) ) {
    ...
    utm_sustainable_set_last_upsert_result_meta(
        array(
            'action' => $is_new_page ? 'inserted' : 'updated',
            'reason' => '',
        )
    );
...
/**
 * Store metadata about the most recent upsert call in this request.
 *
 * @param array $meta Upsert metadata.
 * @return void
 */
function utm_sustainable_set_last_upsert_result_meta( $meta ) {
    $GLOBALS['utm_sustainable_last_upsert_meta'] = is_array( $meta ) ? $meta : array();
}

/**
 * Get metadata about the most recent upsert call in this request.
 *
 * @return array
 */
function utm_sustainable_get_last_upsert_result_meta() {
    if ( isset( $GLOBALS['utm_sustainable_last_upsert_meta'] ) && is_array( $GLOBALS['utm_sustainable_last_upsert_meta'] ) ) {
        return $GLOBALS['utm_sustainable_last_upsert_meta'];
    }
