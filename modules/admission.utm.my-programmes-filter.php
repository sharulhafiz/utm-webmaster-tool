 ...
 *
 * Uses shared context validation from context.php + additional path restriction
 * to avoid redeclaring the utm_admission_is_allowed_context() function.
 */

// First, use the shared context validator from context.php
if ( ! utm_admission_is_allowed_context() ) {
    return;
}

// Additional path restriction: only on /new2024/ path
$home_path = wp_parse_url( home_url( '/' ), PHP_URL_PATH );
$normalized_path = '/' . trim( (string) $home_path, '/' ) . '/';
if ( 0 !== strpos( $normalized_path, '/new2024/' ) ) {
    return;
}
